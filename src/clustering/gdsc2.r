...
## elbow method
fviz_nbclust(t(dfGdsc), kmeans, method = "wss") + labs(subtitle = "Elbow method")
...
basic <- ggplot(results, aes(x=statistic, y=padj)) +
  ...
  geom_point(aes(colour=as.factor(classifier)), size=2.5) +
  theme(legend.position="none") +
  scale_colour_manual(values=c("#2565ae", "#ff0000")) +
  theme(axis.title=element_text(size=15), axis.text=element_text(size=12)) +
  theme(panel.grid.minor=element_blank(), panel.grid.major=element_blank()) +
  labs(x="Test statistic", y=bquote('-'*~Log[10]*' FDR'))
basic
